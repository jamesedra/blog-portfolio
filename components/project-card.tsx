...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <div className={`max-w-lg items-center justify-center`}>
      <div
        className={`${height} rounded-lg relative group`}
        style={{
      ...
      <div className="text-white mt-3 py-0 px-1">
        <h5 className="font-semibold">{title}</h5>
        <div className="flex flex-wrap pb-1">
          {tags.map((tag, index) => (
            <span
              key={index}
              className="bg-stone-800 text-stone-300 text-xs font-light rounded-lg px-2 mr-2 mt-2"
            >
              {tag}
            </span>
          ))}
